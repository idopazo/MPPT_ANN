...
indicespredicciones2=range(210500,214500)
model=keras.models.load_model('todssdatos_5_sigmoid_Nadam.keras')
data_V=curvavsample
...
plt.figure()
plt.plot(range(len(predicciones1)), predicciones1,label="predicciones")
plt.xlabel('Instante')
...
vpredicha1.insert(0,vpredicha1[0])
plt.figure()
plt.plot(range(len(predicciones1)), vpredicha1,label="predicciones",linewidth="1")
plt.xlabel('Instante')
...
plt.plot(range(len(pruebaresults_Vmp.values[indicespredicciones])), pruebaresults_Vmp.values[indicespredicciones],label="Real",linewidth="1")
plt.xlabel('Instante')
plt.ylabel('Tensión[V] ')
plt.legend()
plt.title('Tensión')
...
maev1 = mean_absolute_error(pruebaresults_Vmp.values[indicespredicciones], vpredicha1)
meapv1=np.mean( np.abs((pruebaresults_Vmp.values[indicespredicciones]-vpredicha1)/pruebaresults_Vmp.values[indicespredicciones]))
energiareal1=sum(array_verdadero*0.004)
energiareal1=energiareal1/3600
energiaproducido1=sum(predarray*0.004)
...
predicciones2.append(primerav*resultado_interpolacion)
for muestra_i in data_I[1:]:
        # Predicción con el modelo de Keras    
        resultado_interpolacion = interpolar(data_V, muestra_i, prediccion_anteriorV)
...
plt.figure()
plt.plot(range(len(predicciones2)), predicciones2,label="predicciones")
plt.xlabel('Instante')
...
plt.figure()
plt.plot(range(len(predicciones2)), vpredicha2,label="predicciones",linewidth="1")
plt.xlabel('Instante')
...
plt.plot(range(len(pruebaresults_Vmp.values[indicespredicciones2])), pruebaresults_Vmp.values[indicespredicciones2],label="Real",linewidth="1")
plt.xlabel('Instante')
plt.ylabel('Tensión[V] ')
plt.legend()
plt.title('Tensión')
...
maev2 = mean_absolute_error(pruebaresults_Vmp.values[indicespredicciones2], vpredicha2)
meapv2=np.mean( np.abs((pruebaresults_Vmp.values[indicespredicciones2]-vpredicha2)/pruebaresults_Vmp.values[indicespredicciones2]))
