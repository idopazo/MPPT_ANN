...
for (frame_ini, frame_fin, delta_v) in tramos:
    ...
    eff_pot_po_fijo = pot_po_fijo.sum() / prueba_pmp[frame_ini:frame_fin:1].sum()
    # Calcula eff utilizando pot_ann en lugar de pot_mod
    eff_pot_po = pot_po.sum() / prueba_pmp[frame_ini:frame_fin:1].sum()
    eff_pot_ann=pot_ann.sum() / prueba_pmp[frame_ini:frame_fin:1].sum()
