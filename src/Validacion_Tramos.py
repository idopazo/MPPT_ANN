...
model=keras.models.load_model('Modelos/intento_rms_loss_mae.keras')
data_V=curvavsample
...
plt.figure().set_size_inches(10.35,6.88)
...
plt.xlabel('Instante', fontsize=18)
plt.ylabel('Potencia[W] ', fontsize=18)
plt.legend(fontsize=14)
plt.title('Potencia',fontsize=20)
plt.tick_params(axis='both', which='major', labelsize=14)
plt.show()
energiareal1=sum(array_verdadero*0.004)
eff=sum(predicciones1)/sum(array_verdadero)
print(f'eff={eff:.1%}')
vpredicha_flat=flattened_data = [float(item) for sublist in vpredicha1 for item in (sublist if isinstance(sublist, list) else [sublist])]
plt.figure().set_size_inches(10.35,6.88)
plt.plot(x_values, vpredicha_flat,label="Predicciones",linewidth="0.9")
plt.xlabel('Instante')
...
plt.plot(x_values, pruebaresults_Vmp[indicespredicciones],label="Real",linewidth="0.9")
plt.xlabel('Instante', fontsize=18)
plt.ylabel('Tensión [V]', fontsize=18)
plt.legend(fontsize=14)
plt.title('Tensión',fontsize=20)
plt.tick_params(axis='both', which='major', labelsize=14)
plt.show()
#%%
